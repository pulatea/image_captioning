...
class CaptionGenerator(BaseCaptionGenerator):
    ...
    def forward(self, encoded_image, caption_indices, hidden_state=None):
        ...
        """
        # now embeddings are only word embeddings, and not as before [image_encoded, embedding1, embedding2, ..., embeddingN]
        embeddings = self.embedding(caption_indices)

        if hidden_state is None and encoded_image is not None:
            h_0 = torch.tanh(encoded_image).unsqueeze(0).repeat(self.num_layers, 1, 1).contiguous()
    ...
    def generate_caption_indices(self, encoded_image, sos_token_index, eos_token_index, max_length):
        ...
        # start from the word embeddings (start of sentence)
        caption_indices = [sos_token_index]

        predicted_index = torch.LongTensor([sos_token_index]).to(encoded_image.device).unsqueeze(0)  # [1, 1]

        output = self.forward(encoded_image, caption_indices=predicted_index, hidden_state=None)
